...
- After fatal-clean safe-block reconciliation, the indexer produces `fame-landing-defi-snapshot-v1` locally from one captured reserve set plus bounded block-pinned marketplace/direct-pool reads. Every leaf is either current at that safe block or explicitly unavailable; leaf failures never reuse an older value and are retried on the next pass.
