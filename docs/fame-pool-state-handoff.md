...
- The Lambda bundle also copies the reviewed landing authority artifact. Capability rows fail closed: a quote is enabled only when every route leg has an approved evaluator. Direct-FAME legs use indexed constant-product reserves. The fixed USDC routes use the tracked-only `aerodrome-v2-usdc-weth` connector through one producer-time safe-block reserve read, Aerodrome's contract-ordered volatile fee math, and one batched final `getAmountOut` validation. The runtime adapter rejects every pool, address, token direction, and definition outside that fixed authority. The connector is not indexed or written to DynamoDB. Any missing state, failed validation, or result mismatch makes only the affected current leaf unavailable.
